...
sed -i 's/(FE)/  0  /g' result_cs.txt
sed -i 's/(I)/ 0 /g' result_cs.txt
awk '{s=$3*4+$6*4+$9*4+$12*3+$15*3+$18*3+$21*1+$24*1+$27*1}{r=s/24}{print r}' result_cs.txt >s1_sgpa.txt
awk '{print $1}' result_cs.txt >reg.txt
paste -d ' \t ' reg.txt s1_sgpa.txt >res.txt
awk 'NR==FNR {seen[$3]=1; next} seen[$1]==1 {seen[$2]++; print $2}' s4d.txt res.txt >s1sgpa.txt
awk '{print $3}' s4d.txt >reg.txt
paste -d ' \t ' reg.txt s1sgpa.txt >s1final.txt
pdftotext -nopgbrk s2results.pdf
grep --no-group-separator -A3 'CHN17CS' s2results.txt | tr '\n' ' ' | sed 's/\ CHN/\nCHN/g' > results2.txt
